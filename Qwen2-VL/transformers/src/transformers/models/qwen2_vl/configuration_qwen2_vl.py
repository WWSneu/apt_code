    ...
    def __init__(
        ...
        **kwargs,
        ...
        # APT specific parameters
         # 是否启用 Adaptive Patch Tokenizer（APT）
        self.use_apt = kwargs.pop("use_apt", False)
        # APT 基础 patch 尺寸
        self.apt_base_patch = kwargs.pop("apt_base_patch", 14)
        # APT 多尺度数量（例如 3 表示 14, 28, 56）
        self.apt_num_scales = kwargs.pop("apt_num_scales", 3)
        # 每个尺度对应的阈值（num_scales - 1 个值）
        self.apt_thresholds = kwargs.pop("apt_thresholds", [4.0, 6.0])
        self.apt_norm_mean = kwargs.pop("apt_norm_mean", [0.485, 0.456, 0.406])
        self.apt_norm_std = kwargs.pop("apt_norm_std", [0.229, 0.224, 0.225])
        # 选择打分方法："entropy", "laplacian", "upsample_mse" 等
        self.apt_method = kwargs.pop("apt_method", "entropy")
        # RoPE 位置编码常数（APT 用于2D旋转编码）
        self.apt_pos_rope_theta = kwargs.pop("apt_pos_rope_theta", 10000.0)
        # 启用APT时一般关闭 spatial merge（防止补丁聚合）
        if self.use_apt:
