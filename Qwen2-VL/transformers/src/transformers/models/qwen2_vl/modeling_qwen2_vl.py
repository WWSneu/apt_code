...
class PatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # eval_logger.info(f"PatchEmbed input hidden_states shape: {hidden_states.shape}")
...
# class Qwen2APTVisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def rot_pos_emb(self, grid_thw, output_dict):
        pos_ids = []
        for t, h, w in grid_thw:
            # hpos_ids = torch.arange(h)
            # eval_logger.info("hpos_ids = torch.arange(h) : {}", hpos_ids)
            # hpos_ids = hpos_ids.unsqueeze(1)
            # eval_logger.info("hpos_ids.unsqueeze(1): {}", hpos_ids)
            # hpos_ids = hpos_ids.expand(-1, w)
            # eval_logger.info("hpos_ids.expand(-1, w): {}", hpos_ids)
            ...
            # eval_logger.info("hpos_ids reshaped: {}", hpos_ids)
            hpos_ids = hpos_ids.permute(0, 2, 1, 3)
            # eval_logger.info("hpos_ids permuted: {}", hpos_ids)
            hpos_ids = hpos_ids.flatten()
            # eval_logger.info("hpos_ids flattened: {}", hpos_ids)
            # wpos_ids = torch.arange(w)
            # eval_logger.info("wpos_ids = torch.arange(w): {}", wpos_ids)
            # wpos_ids = wpos_ids.unsqueeze(0)
            # eval_logger.info("wpos_ids.unsqueeze(0): {}", wpos_ids)
            # wpos_ids = wpos_ids.expand(h, -1)
            # eval_logger.info("wpos_ids.expand(h, -1): {}", wpos_ids)
            ...
            # eval_logger.info("wpos_ids reshaped: {}", wpos_ids)
            wpos_ids = wpos_ids.permute(0, 2, 1, 3)
            # eval_logger.info("wpos_ids permuted: {}", wpos_ids)
            wpos_ids = wpos_ids.flatten()
            # eval_logger.info("wpos_ids flattened: {}", wpos_ids)
            # eval_logger.info("hpos_ids: {}, wpos_ids: {}", hpos_ids, wpos_ids)
            pos_ids.append(torch.stack([hpos_ids, wpos_ids], dim=-1).repeat(t, 1))
            # eval_logger.info("pos_ids shape: {}", pos_ids[-1].shape)
            # eval_logger.info("pos_ids[-1]: {}", pos_ids[-1])
            # eval_logger.info("pos_ids: {}, pos_ids_lens: {}", pos_ids[-1], pos_ids[-1].shape)
        pos_ids = torch.cat(pos_ids, dim=0)
        # eval_logger.info("Final pos_ids shape: {}", pos_ids.shape)
        # eval_logger.info("Final pos_ids: {}", pos_ids)
        # ==============================================================================
    ...
    def vectorized_merge_keep_order(self, hidden_states, masks, grid_thw):
        ...
        return final_output
    def vectorized_merge_keep_order_lanczos(self, hidden_states, masks, grid_thw):
        """与 vectorized_merge_keep_order 相同逻辑，但 Scale-2 特征改用 Lanczos 下采样."""
        t, h, w = grid_thw[0]
        if (h % 2) != 0 or (w % 2) != 0:
            raise ValueError("Lanczos 下采样要求 h 和 w 都能被 2 整除")
        ...
        if N != t * h * w:
            raise ValueError("hidden_states 的长度必须等于 t*h*w")
        ...
        feat_s2 = self._lanczos_downsample(hidden_states, t, h, w, scale=2)

        if (h % 4) != 0 or (w % 4) != 0:
            raise ValueError("Lanczos Scale-3 下采样要求 h 和 w 都能被 4 整除")
        feat_s3 = self._lanczos_downsample(hidden_states, t, h, w, scale=4)
    ...
    def _lanczos_downsample(self, hidden_states, t, h, w, scale):
        ...
        输入: [N, 1176]
        N = t * h * w
        1176 = Channel(3) * Temporal(2) * Patch(14) * Patch(14)
        """
        try:
        ...
        # 1. 解包 (Unpack)
        # 按照 transpose 的顺序: C, T, P, P
        patch_size = 14
        temporal_size = 2
        channels = 3
        
        # [t, h, w, 3, 2, 14, 14]
        x = hidden_states.view(t, h, w, channels, temporal_size, patch_size, patch_size)

        # 2. 拼成大图 (Reconstruct Full Image)
        # 我们需要把 t 和 temporal_size 视为 Batch 维度（因为它们不参与空间缩放）
        # 目标: [Batch, Channel, Height, Width]
        # Batch = t * 2
        # Channel = 3
        # Height = h * 14
        # Width = w * 14
        ...
        # permute 顺序: 
        # 原始: 0(t), 1(h), 2(w), 3(C), 4(T_ps), 5(ph), 6(pw)
        # 目标: 0(t), 4(T_ps), 3(C), 1(h), 5(ph), 2(w), 6(pw)
        full_img = x.permute(0, 4, 3, 1, 5, 2, 6).reshape(t * temporal_size, channels, h * patch_size, w * patch_size)
        ...
        # 拆分 Grid
        # [t*2, 3, h_new*14, w_new*14] -> [t, 2, 3, h_new, 14, w_new, 14]
        x_new = downsampled_img.view(t, temporal_size, channels, h_new, patch_size, w_new, patch_size)
        
        # Permute 回原始的内存布局: [t, h, w, C, T, P, P]
        # 当前: 0(t), 1(T), 2(C), 3(h), 4(ph), 5(w), 6(pw)
        # 目标: 0(t), 3(h), 5(w), 2(C), 1(T), 4(ph), 6(pw)
        x_new = x_new.permute(0, 3, 5, 2, 1, 4, 6).contiguous()
        
        # Flatten
        # [N_new, 1176]
        feat_s_out = x_new.view(-1, 1176).to(dtype=dtype)

        return feat_s_out

    
    def _save_debug_image(self, tensor_img, folder="Render", max_keep=3000):
        """
        保存 Tensor 为图片用于 Debug。
        策略：如果文件夹内图片数量达到 max_keep，则停止保存新图片（不删除旧图片）。
        """
        import os
        import time
        ...
        # 2. 检查数量限制 (Capacity Cap)
        # 获取文件夹内所有的 png 文件
        current_files = [f for f in os.listdir(folder) if f.endswith('.png')]
        
        # 如果已满，直接返回，不做任何操作
        if len(current_files) >= max_keep:
            return

        # 3. 准备图片数据
        # 如果是 Batch 模式 [B, C, H, W]，只存第一张
        img_to_save = tensor_img[0] if tensor_img.dim() == 4 else tensor_img
        
        # 克隆并转为 float，移至 CPU
        img_to_save = img_to_save.detach().float().cpu()

        # 4. 归一化 (Min-Max Normalize)
        # 这一步对于可视化 feature map 至关重要，否则图片很可能全黑或全白
        img_min = img_to_save.min()
        img_max = img_to_save.max()
        if img_max > img_min:
        ...
        nano = str(time.time_ns())[-6:] 
        filename = f"debug_{timestamp}_{nano}.png"
    ...
    def vectorized_merge_keep_order_bicubic(self, hidden_states, masks, grid_thw):
        """与 vectorized_merge_keep_order 相同逻辑，但 Scale-2/3 特征改用 Bicubic 下采样."""
        t, h, w = grid_thw[0]
        # ... (前置检查逻辑不变) ...
        if (h % 2) != 0 or (w % 2) != 0:
            raise ValueError("下采样要求 h 和 w 都能被 2 整除")
        ...
        # === 修改点：调用 _bicubic_downsample ===
        feat_s2 = self._bicubic_downsample(hidden_states, t, h, w, scale=2)

        if (h % 4) != 0 or (w % 4) != 0:
            raise ValueError("Scale-3 下采样要求 h 和 w 都能被 4 整除")
        feat_s3 = self._bicubic_downsample(hidden_states, t, h, w, scale=4)
        # ======================================
    ...
    def _bicubic_downsample(self, hidden_states, t, h, w, scale):
        """
        使用 Bicubic 插值进行下采样。
        输入: [N, 1176]
        结构: 1176 = Channel(3) * Temporal(2) * Patch(14) * Patch(14)
        """
        # 使用 PyTorch 原生函数
        import torch.nn.functional as F

        if (h % scale) != 0 or (w % scale) != 0:
            raise ValueError(f"下采样要求 h、w 能被 {scale} 整除")
        ...
        # View: [t, h, w, 3, 2, 14, 14]
        x = hidden_states.view(t, h, w, channels, temporal_size, patch_size, patch_size)

        # 2. 拼成大图 (Reconstruct Full Image)
        # 我们将 t 和 temporal_size 放到 Batch 维度，因为它们不需要跨时间插值，而是独立插值。
        # 目标: [Batch, Channel, Height, Width]
        # Batch = t * 2
        # Channel = 3
        # Height = h * 14
        ...
        # Permute 顺序: 
        # 原始: 0(t), 1(h), 2(w), 3(C), 4(T), 5(ph), 6(pw)
        # 目标: 0(t), 4(T), 3(C), 1(h), 5(ph), 2(w), 6(pw)
        full_img = x.permute(0, 4, 3, 1, 5, 2, 6).reshape(t * temporal_size, channels, h * patch_size, w * patch_size)
        self._save_debug_image(full_img, folder="Render/Bicubic_FullImg")

        # 3. Bicubic 下采样
        target_h_pixels = (h // scale) * patch_size
        target_w_pixels = (w // scale) * patch_size
        
        # F.interpolate 需要 Float 输入
        full_img_float = full_img.float()
        
        # 执行插值
        # antialias=True 是比较推荐的选项（PyTorch 1.11+ 支持），可以减少混叠
        downsampled_img = F.interpolate(
            full_img_float, 
            size=(target_h_pixels, target_w_pixels),
            mode='bicubic',
            align_corners=False,
            antialias=True
        )
    ...
    def _compute_z_order_keys(self, indices, width):
        ...
        Formula: Block_ID * 4 + Inner_ID
        """
        # 1. 解析坐标 (y, x)
        y = indices // width
        x = indices % width
        
        # 2. 计算宏块坐标 (Block Coordinates)
        by = y // 2
        bx = x // 2
        
        # 3. 计算块内坐标 (Inner Coordinates)
        iy = y % 2
        ix = x % 2
        
        # 4. 计算 Z-Order Key
        # 宏观上：Block 按照 (width // 2) 的步长排列
        # 微观上：Block 内部按照 0(TL), 1(TR), 2(BL), 3(BR) 排列
        block_width = width // 2
        block_id = by * block_width + bx
        inner_id = iy * 2 + ix
        
        return block_id * 4 + inner_id

    def vectorized_merge_keep_order_block_major_bicubic(self, hidden_states, masks, grid_thw):
        """
        Block-Major 版本：
        1. 先将 hidden_states (Block优先) 还原成一张完整的全分辨率大图 (Full Image)。
        2. 基于这张大图，分别做 Bicubic 下采样得到 Scale-2 和 Scale-3 的特征。
        """
        t, h, w = grid_thw[0]
        
        # 1. 基础检查
        if (h % 4) != 0 or (w % 4) != 0:
            raise ValueError("为了同时支持 Scale-2/3，h 和 w 必须能被 4 整除")
        ...
        original_indices = torch.arange(N, device=device)
        # indices_blk = original_indices.view(-1, 4)
        # eval_logger.info(f"indices_blk: {indices_blk.shape[0]}")

        # original_indices = original_indices.view(t, h // 2, 2, w // 2, 2)
        # original_indices = original_indices.permute(0, 1, 3, 2, 4).contiguous()
        # original_indices = original_indices.view(-1)

        eval_logger.info(f"original_indices: {original_indices}")
        # 2. 准备 Mask 和索引视图 (Block-Major 假设)
        # 这里的 view(-1, 4) 对应内存中连续的 4 个 token (2x2 Block)
        hidden_blk = hidden_states.view(-1, 4, C)
        # eval_logger.info(f"hidden_blk: {hidden_blk.shape[0]}")
        indices_blk = original_indices.view(-1, 4)
        # eval_logger.info(f"indices_blk: {indices_blk}")
        mask_blk_vals = masks[1].view(-1, 4)[:, 0]
        # eval_logger.info(f"mask_blk_vals: {mask_blk_vals}")
        # =========================================================
        ...
        # B. 基于同一张大图，进行不同尺度的下采样
        # Scale 2: 目标尺寸 (h/2, w/2)
        feat_s2 = self._downsample_from_full_img(full_img, t, h, w, scale=2, target_dtype=hidden_states.dtype)
        
        # Scale 3: 目标尺寸 (h/4, w/4)
        feat_s3 = self._downsample_from_full_img(full_img, t, h, w, scale=4, target_dtype=hidden_states.dtype)

        # =========================================================

        # 3. 索引计算 (用于最后的排序恢复)
        # Scale 2 索引: 取每个 Block (2x2) 的左上角 (第0个元素)
        h_blk, w_blk = h // 2, w // 2
        idx_s2 = indices_blk[:, 0] # [N/4]
        # eval_logger.info(f"idx_s2: {idx_s2}")
        # Scale 3 索引: 在 Scale 2 的 grid 上再取 2x2 的左上角
        # 注意：idx_s2 本身已经是按宏块 Raster Scan 排列的
        idx_s2_spatial = idx_s2.view(t, h_blk, w_blk)
        # eval_logger.info(f"idx_s2_spatial: {idx_s2_spatial}")
        idx_s3 = idx_s2_spatial[:, 0::2, 0::2].flatten()
        # eval_logger.info(f"idx_s3: {idx_s3}")
        # 4. 收集与合并 (Gather & Merge)
        masks[1] = masks[1].view(-1)
        
        # Mask = 1: 保留原始 (从原 hidden_states 取，或者你也可以从 full_img 还原，但原数据不动最好)
        mask_1 = (masks[1] == 1)
        out_1 = hidden_states[mask_1]
        out_idx_1 = original_indices[mask_1]
    ...
    def _reconstruct_full_img_block_major(self, hidden_states, t, h, w):
        """
        将 Block-Major 排列的 hidden_states 还原为完整图像。
        Input: [N, 1176] (Block-Major: 每4个token组成一个2x2块)
        Output: [Batch, 3, H_pixel, W_pixel] (Batch = t * temporal_size)
        """
        # 1. 结构参数
        patch_size = 14
        temporal_size = 2
        ...
        # 2. 多级 Reshape (这是 Block-Major 的核心拼图逻辑)
        # hidden_states: [t * h * w, C]
        # h 和 w 是 Token 数量。由于 Block-Major，我们按 (h/2, w/2) 个 "大块" 来切分。
        # view: [t, grid_h, grid_w, block_h, block_w, C]
        # block_h=2, block_w=2 对应那 "4" 个 token 的内部排列 (0,1 为第一行, 2,3 为第二行)
        ...
        # 3. Permute 拼成大图
        # 目标: [t, T, C, (GridH * BlockH * PH), (GridW * BlockW * PW)]
        # 原始维度索引:
        # 0:t, 1:gh, 2:gw, 3:bh, 4:bw, 5:C, 6:T, 7:ph, 8:pw
        
        # 目标顺序:
        # 0:t
        # 6:T
        # 5:C
        # 1:gh -> 3:bh -> 7:ph (高度方向融合)
        # 2:gw -> 4:bw -> 8:pw (宽度方向融合)
        
        full_img = x.permute(0, 6, 5, 1, 3, 7, 2, 4, 8)
        
        # 4. Reshape 融合
        # [t * T, C, h * 14, w * 14]
        full_img = full_img.reshape(
            t * temporal_size, 
            channels, 
            h * patch_size, 
            w * patch_size
        ...
        # 可选：在这里插入 Debug 保存图片
        self._save_debug_image(full_img, folder="Render_BlockMajor_Full", max_keep=30)
        
        return full_img

    def _downsample_from_full_img(self, full_img, t, h, w, scale, target_dtype):
        """
        通用的下采样函数：输入已经是完整大图。
        Output: [N_new, 1176] (Row-Major / Raster Scan Order)
        注意：不论输入是 Block-Major 还是 Row-Major，输出通常都需要是 Raster Scan，
        以便后续 Transformer 层处理。
        """
        import torch.nn.functional as F
        ...
        # 1. 计算目标像素尺寸
        target_h_px = (h // scale) * patch_size
        target_w_px = (w // scale) * patch_size
        
        # 2. Bicubic 下采样
        # 输入必须是 float
        img_float = full_img.float()
        
        downsampled = F.interpolate(
            img_float,
            size=(target_h_px, target_w_px),
            mode='bicubic',
            align_corners=False,
            antialias=True
        )
        self._save_debug_image(downsampled, folder="Render_BlockMajor_Full", max_keep=3000)

        # 3. 切分回 Patch (Repack to Raster Scan / Row-Major)
        # 为什么切回 Row-Major？因为这是 Transformer 的标准输入格式。
        # 即使之前的输入是 Block-Major，经过合并后，通常这层输出会被视为一个新的 Grid，
        # 这个新 Grid 按标准行优先排列。
        
        h_new = h // scale
        w_new = w // scale
        
        # [Batch, C, H, W] -> [t, T, C, h_new, p, w_new, p]
        x_new = downsampled.view(t, temporal_size, channels, h_new, patch_size, w_new, patch_size)
        
        # Permute 回 1176 内存布局: [t, h, w, C, T, P, P] (标准的行优先布局)
        # 原: 0(t), 1(T), 2(C), 3(h), 4(ph), 5(w), 6(pw)
        # 标: 0(t), 3(h), 5(w), 2(C), 1(T), 4(ph), 6(pw)
        x_new = x_new.permute(0, 3, 5, 2, 1, 4, 6).contiguous()

        # Flatten
        return x_new.view(-1, channels * temporal_size * patch_size * patch_size).to(dtype=target_dtype)
    ...
    def vectorized_merge_pos_embeds_z_order(self, position_embeddings, masks, grid_thw):
        """
        与 vectorized_merge_pos_embeds 逻辑一致，但使用 Z-Order (2x2 Block优先) 进行排序。
        确保 Pos Embed 与 Image Token 的排列顺序完全匹配。
        """
        # -------------------------------------------------
        # 0. 预处理
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 1. 准备视图 & 2. 计算特征 Candidates
        # (这部分逻辑保持不变，因为平均池化不依赖排序方式)
        # -------------------------------------------------
        pos_blk = pos_all.view(-1, 4, 2, C)
        indices_blk = original_indices.view(-1, 4)
        mask_blk_vals = masks[1].view(-1, 4)[:, 0]

        # Scale 2 (Mean Pooling)
        feat_s2 = pos_blk.mean(dim=1)
        
        # Scale 3 (Mean Pooling on Scale 2)
        h_blk, w_blk = h // 2, w // 2
        feat_s2_spatial = feat_s2.view(t, h_blk, w_blk, 2, C)
        
        # 调整维度以适应 avg_pool2d: (t, 2*C, h, w)
        feat_for_pool = feat_s2_spatial.permute(0, 3, 4, 1, 2).reshape(t, 2 * C, h_blk, w_blk)
        feat_pooled = torch.nn.functional.avg_pool2d(feat_for_pool, kernel_size=2, stride=2)
        ...
        # -------------------------------------------------
        # 5. 恢复顺序 (Restoring Order with Z-Curve)
        # -------------------------------------------------
        all_feats = torch.cat([out_1, out_2, out_3], dim=0) # (Total, 2, C)
        all_indices = torch.cat([out_idx_1, out_idx_2, out_idx_3], dim=0)

        # ★★★ 关键修改：计算 Z-Order Keys 并排序 ★★★
        z_keys = self._compute_z_order_keys(all_indices, w)
        sort_order = torch.argsort(z_keys)
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config: Qwen2VLConfig):
        super().__init__(config)
        # if getattr(config.vision_config, "use_apt", False):
        #     self.visual = Qwen2APTVisionTransformerPretrainedModel._from_config(config.vision_config)
        # else:
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None, output_dict: Optional[dict] = None, images_output: Optional[dict] = None):
        """
        ...
        Args:
            ...
            output_dict (`dict`, *optional*):
                Additional data to pass to the visual model.
            images_output (`dict`, *optional*):
                A dict to store outputs from the visual model.
        """
        # eval_logger.info("pixel_values shape: {}", pixel_values.shape)
        # eval_logger.info("image_grid_thw: {}", image_grid_thw)
    ...
    def get_placeholder_mask(
        ...
        if input_ids is None:
            # eval_logger.info("input_ids is None, calculating special token mask from inputs_embeds.")
            ...
            # eval_logger.info("input_ids is provided, calculating special token mask from input_ids.")
    ...
    def forward(
        ...
        images_output: Optional[dict] = None,
        ...
        if position_ids is None:
            if self.rope_deltas is None or cache_position is None or cache_position[0] == 0:
                position_ids, rope_deltas = self.get_rope_index(
                    input_ids, image_grid, video_grid_thw, attention_mask
                )
