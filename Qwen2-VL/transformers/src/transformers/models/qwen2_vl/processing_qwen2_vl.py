    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2VLProcessorKwargs],
        ...
        if images is not None:
            # eval_logger.info("images: {}", images.shape if hasattr(images, "shape") else images)
            # eval_logger.info("image shapes: {}", images[0].shape)
            image_inputs, output_dict, images_outputs, grid_new_thw = self.image_processor(images=images, **output_kwargs["images_kwargs"])
            # eval_logger.info("image_inputs keys: {}, image_input values: {}, image_input: {}", image_inputs.keys(), image_inputs.values(), len(image_inputs))
            # eval_logger.info("image_inputs output_dict{}", image_inputs["output"][0])
            image_grid_thw = image_inputs["image_grid_thw"]

                # print(item)
                # print(image_inputs[item].shape)
            # image_inputs = BatchFeature(data={"pixel_values": image_inputs["pixel_values"], "image_grid_thw": image_inputs["image_grid_thw"]}, tensor_type=return_tensors)
            # image_inputs = BatchFeature(data={"output": image_inputs["output"]}, tensor_type=return_tensors)
        ...
        #     eval_logger.info("image_inputs item shape: {}", image_inputs[item].shape)
        return BatchFeature(data={**text_inputs, **image_inputs, **videos_inputs}, tensor_type=return_tensors), output_dict, images_outputs, grid_new_thw
